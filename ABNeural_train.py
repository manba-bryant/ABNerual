...
from generate_dataset import generate_pairs, dataset_generation, zero_padded_adjmat, feature_vector
import config
import matplotlib.pyplot as plt
import numpy as np
from sklearn.metrics import roc_curve, auc

# from ABNeural.config import Gemini_model_save_path
...
class MyModel(tf.keras.Model):
    def __init__(self):
        ...
        self.W1 = tf.Variable(tf.random.uniform(
            [config.embedding_size, config.ABNeural_feature_size], maxval=0.1, dtype=tf.float32))
...
def train():
    ...
    model = MyModel()
    model.build([(None, config.max_nodes, config.max_nodes), (None, config.max_nodes, config.ABNeural_feature_size),
                (None, config.max_nodes, config.max_nodes), (None, config.max_nodes, config.ABNeural_feature_size)])
    model.summary()
    max_auc = 0
    train_loss = []
    valid_loss = []
    ...
    for epoch in range(config.epochs):
        ...
        epoch_accuracy_avg = tf.keras.metrics.BinaryAccuracy(threshold=0.4)
        epoch_auc_avg = tf.keras.metrics.AUC()
        step = 0
        ...
        if v_auc > max_auc:
            model.save(config.ABNeural_model_save_path, save_format='tf')
            max_auc = v_auc
    test(model)
    plt.figure(figsize=(5, 4))
    ...
    plt.ylabel("loss")
    plt.legend()
    plt.savefig(config.ABNeural_figure_save_path+"loss.png")
    ...
    plt.ylabel("accuracy")
    plt.legend()
    plt.savefig(config.ABNeural_figure_save_path + "accuracy.png")
    ...
    plt.ylabel("AUC")
    plt.legend()
    plt.savefig(config.ABNeural_figure_save_path + "auc.png")
