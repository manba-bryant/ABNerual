        ...
        for v in config.version:
            for c in config.compiler:
                for o in config.optimizer:
                    ...
                    filepath = os.path.join(config.dir_name, filename + ".cfg")
...
def dataset_split(all_function_dict):
    ...
    with open("dataset/ABNeural/train", "wb") as f:
...
def generate_pairs(type):
    ...
    filepath = config.ABNeural_dataset_dir + type.decode()
    with open(filepath, "rb") as f:
